  ...
  return (
    <div className="ar-root" data-theme={theme}>
      ...
      <section>
        ...
        <p className="lede">Six of the twelve slots never change ability, lap after lap: Creeping Terror, Discharge, three copies of Eradicate, and Death Field. The other six aren't random — they're filled by a strict priority order, and most of the "which ability goes here" variance in the reference grid comes from just one of those six slots.</p>

        <figure>
          <svg viewBox="0 0 820 230" role="img" aria-label="Twelve-slot cycle: Creeping Terror, Discharge, then a flexible slot, Eradicate, three more flexible slots, Eradicate, a flexible slot, Death Field, a flexible slot, Eradicate. Six slots are fixed and six are filled by priority: Assassinate first, then Leeching Strike, then Thrash, then Saber Strike.">
            ...
            <g><title>Creeping Terror — fixed</title><use href="#h-ct" x="24" y="26" width="44" height="44"/></g>
            <g><title>Discharge — fixed</title><use href="#h-dc" x="90" y="26" width="44" height="44"/></g>
            ...
            <g><title>Eradicate — fixed, slot 4</title><use href="#h-er" x="222" y="26" width="44" height="44"/></g>
            ...
            <g><title>Eradicate — fixed, slot 8</title><use href="#h-er" x="486" y="26" width="44" height="44"/></g>
            ...
            <g><title>Death Field — fixed (slot 10 in parse 2, slot 5 in parse 1 — see the style-difference note below)</title><use href="#h-df" x="618" y="26" width="44" height="44"/></g>
            ...
            <g><title>Eradicate — fixed, slot 12</title><use href="#h-er" x="750" y="26" width="44" height="44"/></g>
            ...
            <g><title>Assassinate</title><use href="#h-as" x="182" y="150" width="42" height="42"/></g>
            <text className="sv-numb" x="203" y="206" textAnchor="middle" fill="var(--er-f)">1st</text>
            <g><title>Leeching Strike</title><use href="#h-ls" x="282" y="150" width="42" height="42"/></g>
            <text className="sv-numb" x="303" y="206" textAnchor="middle">2nd</text>
            <g><title>Thrash</title><use href="#h-th" x="382" y="150" width="42" height="42"/></g>
          ...
          <figcaption>Solid icons are fixed — identical every lap, both parses. Dashed boxes (F1–F6) are each filled by whichever of the four priority abilities applies. Median lap length 17.09 s across 14 clean laps in parse 2 (was: parse 1's 17.25 s across 15). Death Field's own slot is fixed within each parse but not between them — parse 1 places it at slot 5, right after the opening DoTs; parse 2 delays it to slot 10. Neither is more "correct"; both are internally 100% consistent within their own parse, which is what makes it a style difference rather than an execution gap.</figcaption>
        </figure>

        <p className="lede" style={{ marginBottom: '24px' }}><b>F1</b> is where most of the grid's variety actually lives — Assassinate whenever Bloodletting is up or health allows it, Leeching Strike otherwise. <b>F2–F4</b> are almost always Leeching Strike, then Thrash whenever Leeching Strike is still on its own cooldown. <b>F5</b> is Saber Strike in all but one of the 14 clean laps; <b>F6</b> splits close to evenly between Thrash and Assassinate.</p>
        ...
        <table>
          ...
          <tbody>
            <tr className="hi"><td className="n">1</td><td className="icc"><span className="ic" data-n="Assassinate" role="img" aria-label="Assassinate"><svg viewBox="0 0 50 50"><use href="#h-as"/></svg></span></td><td>Assassinate</td><td className="n">20, 6s CD</td><td>Bloodletting is up, or the target is below 30% health. Take it the moment it's available — a proc left sitting is priority lost, not banked.</td></tr>
            <tr><td className="n">2</td><td className="icc"><span className="ic" data-n="Leeching Strike" role="img" aria-label="Leeching Strike"><svg viewBox="0 0 50 50"><use href="#h-ls"/></svg></span></td><td>Leeching Strike</td><td className="n">13–25, ~12s CD</td><td>The default filler, used on cooldown — its own ~12 s cooldown doesn't divide evenly into the ~17 s macro-cycle, which is why it isn't always up for both of a lap's usual two LS-preferred slots. Cost drops 4 per active DoT, as low as 13 with all three up.</td></tr>
            <tr><td className="n">3</td><td className="icc"><span className="ic" data-n="Thrash" role="img" aria-label="Thrash"><svg viewBox="0 0 50 50"><use href="#h-th"/></svg></span></td><td>Thrash</td><td className="n">20, no CD</td><td>Fills in whenever Leeching Strike is still on cooldown and Force allows.</td></tr>
        ...
        <div className="call">
          <h4>Why Assassinate shows up once some laps, twice others</h4>
          <p>Assassinate's own cooldown is short — measured at a median 5.6–5.7 s below 30% health, in the same range as Eradicate's. Above 30%, though, it can only fire when Bloodletting is up, and Bloodletting's own gate runs on a completely different clock: a ~15 s internal cooldown (measured 14.0–17.9 s between procs across both parses) plus whatever random delay it takes for the 30%-chance roll to land on a periodic tick after that cooldown clears.</p>
          <p>A ~14–18 s proc cycle doesn't divide evenly into a ~17 s macro-cycle. Some laps get exactly one window where a proc is ready; others get none until late, then two clear close together. That mismatch — not inconsistent play — is the entire reason Assassinate's slot count swings between one and two per lap above 30% health, and it's also why the swing disappears completely below 30%, where Assassinate runs on its own clock alone.</p>
        </div>
